      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {!deck ? (
              ...
              <div className="p-8 space-y-6">
                ...
                <div>
                  ...
                  <div 
                    ...
                    className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${fileData ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                  >
                    {fileData ? (
                      <div className="flex flex-col items-center">
                        ...
                        <p className="font-medium text-indigo-700 dark:text-indigo-400">File Ready</p>
                        <p className="text-xs text-indigo-400 mt-1">Click to change resource</p>
                      </div>
                    ) : (
                      <div className="flex flex-col items-center">
                        <svg className="w-12 h-12 text-slate-400 dark:text-slate-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                        <p className="font-medium text-slate-600 dark:text-slate-400">Select Image or PDF Document</p>
                        <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Gemini will analyze the content directly</p>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <aside className="lg:col-span-3 space-y-4 max-h-[calc(100vh-140px)] overflow-y-auto pr-2 no-print custom-scrollbar">
              <h3 className="text-sm font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest px-2 mb-4">Slides</h3>
              {deck.slides.map((s, i) => (
                <button
                  key={i}
